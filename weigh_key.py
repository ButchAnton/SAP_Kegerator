...
#
# keg_weight.py - Weigh a keg via a Wii Balance Board connected
# via bluetooth to a RPi.
#
# Butch Anton
# June, 2015
#
...
#
# Based on work "borrowed" (rhymes with "stolen") from Initial State
# Technologies.
#
...
# --------- User Settings ---------
WEIGHT_SAMPLES = 500
WEIGHT_DIFFERENCE = 1 # number of pounds difference to consider it a change
# ---------------------------------
  ...
  def receive(self):
    ...
    while self.status == "Connected" and not self.processor.done:
      ...
      if intype == INPUT_STATUS:
        ...
        new_weight = self.processor.getWeight()
        # print "old_weight = " + str(self.getOldWeight())
        # print "new_weight = " + str(new_weight)
        if (abs(new_weight - self.getOldWeight()) >= 1.0):
          print "******* Weight has changed, old_wegith = " + str(self.getOldWeight()) + ", new_weight = " + str(new_weight) + ", difference = " + str((new_weight - self.getOldWeight()))
          self.setOldWeight(new_weight)
